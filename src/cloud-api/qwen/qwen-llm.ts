...
const qwenLLMModel = process.env.QWEN_LLM_MODEL || "qwen-plus";
const qwenApiUrl =
  process.env.QWEN_API_URL ||
  "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";

const chatHistoryFileName = `qwen_chat_history_${moment().format(
  "YYYY-MM-DD_HH-mm-ss",
)}.json`;
...
const summaryTextWithLLM: SummaryTextWithLLMFunction = async (
  ...
  if (!qwenApiKey) {
    console.error("Qwen API key is not set. Using original text.");
    return text;
  }
  const response = await axios
    ...
    .catch((error) => {
      console.log("Error during Qwen summary request:", error.message);
      return null;
    });
  if (!response) {
    return text;
  ...
  if (summary) {
    console.log("Qwen summary:", summary);
    return summary;
  } else {
    console.log("No summary returned from Qwen. Using original text.");
    return text;
  }
};
