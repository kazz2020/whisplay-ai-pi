...
const minimaxLLMModel = process.env.MINIMAX_LLM_MODEL || "MiniMax-Text-01";
const minimaxApiUrl =
  process.env.MINIMAX_API_URL || "https://api.minimax.chat/v1/chat/completions";

const chatHistoryFileName = `minimax_chat_history_${moment().format(
  "YYYY-MM-DD_HH-mm-ss",
)}.json`;
...
const chatWithLLMStream: ChatWithLLMStreamFunction = async (
  ...
  if (!minimaxApiKey) {
    console.error("MiniMax API key is not set.");
    return;
  }
...
const summaryTextWithLLM: SummaryTextWithLLMFunction = async (
  ...
  if (!minimaxApiKey) {
    console.error("MiniMax API key is not set. Using original text.");
    return text;
  }
  const response = await axios
    ...
    .catch((error) => {
      console.log("Error during MiniMax summary request:", error.message);
      return null;
    });
  if (!response) {
    return text;
  ...
  if (summary) {
    console.log("MiniMax summary:", summary);
    return summary;
  } else {
    console.log("No summary returned from MiniMax. Using original text.");
    return text;
  }
};
